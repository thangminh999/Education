...
namespace _2011438_HoangNgocMinhThang_VD1
{
    public partial class frmTrungTam : Form
    {
        ...
        private void btnTinhTien_Click(object sender, EventArgs e)
        {
            ...
            if(chkTinHocA.Checked)
                s+=int.Parse(lbiTienTHA.Text.Split(',')[0]);
            if (chkTinHocB.Checked)
                s += int.Parse((lbiTienTHB.Text.Split('.')[0]));
            if (chkTiengAnhA.Checked)
                s += int.Parse(lbiTienTAA.Text.Split('.')[0]);
            if(chkTiengAnhB.Checked)
                s+= int.Parse((lbiTienTAB.Text.Split('.')[0]));
